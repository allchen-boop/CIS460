...
void Player::processInputs(InputBundle &inputs) {
    ...
    if (inputs.flightMode) { // flight mode on
        ...
        if (inputs.wPressed) {
            ...
            m_acceleration = -acceleration * normalize(vec3(m_right.x, 0, m_right.z));
        } else if (inputs.spacePressed && inputs.onGround) {
...
void Player::computePhysics(float dT, const Terrain &terrain, InputBundle &input) {
    ...
    vec3 gravity = vec3(0.f, -9.8f, 0.f); // -9.8 for natural gravity effect
...
bool Player::isOnGround(const Terrain &terrain, InputBundle &input) {
    ...
    for (int x = 0; x <= 1; x++) {
        for (int z = 0; z <= 1; z++) {
            vec3 p = vec3(floor(bottomLeftVertex.x) + x, floor(bottomLeftVertex[1] - 0.005f),
        ...
        for (int z = 0; z >= -1; z--) {
            for (int y = 0; y <= 2; y++) {
                ...
                if (gridMarch(rayOrigin, *rayDirection, terrain, &out_dist, &out_blockHit)) {
                    float distance = glm::min(out_dist - 0.005f, abs(length(this->m_position - vec3(out_blockHit))));
